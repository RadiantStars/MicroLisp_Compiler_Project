...
(defun if-check (e env)
  (if (not (equal (evaluate (cadr e) env) nil))
      (evaluate (caddr e) env)
      (evaluate (cadddr e) env)))

(defun lets-go (e env &optional (ara '()))
  (dolist (l (cadr e))
    (if (typep (cadr l) 'cons)
        (setf (cadr l) (evaluate (cadr l) env)))
    (setf ara (cons (cons (car l) (cadr l)) ara)))
  (setf env (append ara env))
  (eprogn (cddr e) env))
